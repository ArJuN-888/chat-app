...
try
{
    const {username,email,password,studentid,studentname,studentclass}=req.body;

    const parent=await parentModel.findOne({email})
    if(!username || !email || !password || !studentclass || !studentid || !studentname)
    {
    ...
    const newParent=new parentModel({email,password:hashedPassword,username,studentclass,studentid,studentname})
