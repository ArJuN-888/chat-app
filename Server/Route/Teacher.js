...
try
{
    const {username,email,password,teacherid,classname}=req.body;

    const teacher=await teacherModel.findOne({email})
    if( !username || !email || !password || !classname || !teacherid ) 
    {
    ...
    const newTeacher=new teacherModel({email,password:hashedPassword,username,classname,teacherid})
    await newTeacher.save()
    res.json({message:"Faculty Registration Successfull "})
}
catch(error)
{
    return res.status(400).json({message:"Error in Faculty Registration"})
}
...
router.post("/login",async(req,res)=>{
   ...
   return res.status(200).json({message:"Successfully logged-in",token:token,teacherID:teacher._id})
}
catch(error)
{
    return res.status(400).json({message:"Error in Faculty Login!!!"})
}
